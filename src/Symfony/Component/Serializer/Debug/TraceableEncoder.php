...
class TraceableEncoder implements EncoderInterface, DecoderInterface, SerializerAwareInterface
{
    ...
    public function encode(mixed $data, string $format, array $context = []): string
    {
        ...
        if ($traceId = ($context[TraceableSerializer::DEBUG_TRACE_ID] ?? null)) {
            $this->dataCollector->collectEncoding($traceId, $this->encoder::class, $time, $this->scope);
        }
    ...
    public function decode(string $data, string $format, array $context = []): mixed
    {
        ...
        if ($traceId = ($context[TraceableSerializer::DEBUG_TRACE_ID] ?? null)) {
            $this->dataCollector->collectDecoding($traceId, $this->encoder::class, $time, $this->scope);
        }
