...
class SerializerPassTest extends TestCase
{
    ...
    public static function provideDefaultSerializerTagsData(): iterable
    {
        ...
        yield 'include standard with different name' => [
            ['standard' => true, 'serializer' => 'api'],
            ['standard' => true, 'serializer' => 'api'],
            [[]],
            [[]],
        ];
        ...
        yield 'is unique when standard with name' => [
            ['standard' => true, 'serializer' => 'default'],
            ['standard' => true, 'serializer' => 'default'],
            [[]],
            [[]],
        ];
    ...
    {
        yield 'include standard' => [
            ['include_standard_normalizers' => true, 'include_standard_encoders' => true],
            ['standard' => true],
            ['standard' => true],
            [[]],
            [[]],
        ...
        yield 'include standard normalizers only' => [
            ['include_standard_normalizers' => true, 'include_standard_encoders' => false],
            ['standard' => true],
            ['standard' => true],
            [[]],
            [],
        ...
        yield 'include standard encoders only' => [
            ['include_standard_normalizers' => false, 'include_standard_encoders' => true],
            ['standard' => true],
            ['standard' => true],
            [],
            [[]],
        ...
        yield 'include name' => [
            ['include_standard_normalizers' => false, 'include_standard_encoders' => false],
            ['serializer' => 'api'],
            ['serializer' => 'api'],
            [[]],
            [[]],
        ...
        yield 'do not include when include standard not set' => [
            [],
            ['standard' => true],
            ['standard' => true],
            [],
        ...
        yield 'do not include not standard and no name' => [
            ['include_standard_normalizers' => false, 'include_standard_encoders' => false],
            [],
            [],
            [],
    ...
    public function testThrowExceptionWhenNoNormalizersForNamedSerializers()
    {
        ...
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('You must tag at least one service as "serializer.normalizer" with the name "api" to use the "serializer.api" service.');
    ...
    public function testThrowExceptionWhenNoEncodersForNamedSerializers()
    {
        ...
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('You must tag at least one service as "serializer.encoder" with the name "api" to use the "serializer.api" service.');
    ...
    /**
     * @testWith [{}, null, []]
     *           [{"a": true, "b": 3}, null, {"default_context": {"b": 3, "a": true}}]
     *           [{"b": 3, "a": true}, null, {"default_context": {"a": true, "b": 3}}]
     *           [{}, "some.converter", {"name_converter": "some.converter"}]
     *           [{"a": true, "b": 3}, "some.converter", {"default_context": {"b": 3, "a": true}, "name_converter": "some.converter"}]
     */
    public function testNamedSerializersUseSameServicesAsDefaultWhenConfigsAreTheSame(
        ...
        $this->assertEquals([new Reference('n')], $container->getDefinition('serializer.api')->getArgument(0));
        $this->assertEquals([new Reference('e')], $container->getDefinition('serializer.api')->getArgument(1));
    }

    /**
     * @dataProvider provideDifferentNamedSerializerConfigsCases
     */
    public function testNamedSerializersCreateNewServicesForDifferentConfigs(
        array $defaultSerializerDefaultContext,
        ?string $defaultSerializerNameConverter,
        array $namedSerializerConfig,
        ...
        $this->assertEquals([new Reference('n.'.$configHash)], $container->getDefinition('serializer.api')->getArgument(0));
        $this->assertEquals(new Reference($nameConverterId), $container->getDefinition('n.'.$configHash)->getArgument(0));
        $this->assertEquals([new Reference('e.'.$configHash)], $container->getDefinition('serializer.api')->getArgument(1));
        $this->assertEquals(new Reference($nameConverterId), $container->getDefinition('e.'.$configHash)->getArgument(0));
    }
    ...
    public static function provideDifferentNamedSerializerConfigsCases(): iterable
    {
        yield [
            ...
            $nameConverterId = 'serializer.name_converter.metadata_aware',
            ContainerBuilder::hash([['a' => true, 'c' => 3], $nameConverterId]),
        ];
        yield [
            [],
            'some.converter',
            ['name_converter' => null],
            $nameConverterId = 'serializer.name_converter.metadata_aware.'.ContainerBuilder::hash(null),
            ContainerBuilder::hash([[], $nameConverterId]),
        ];
        yield [
            ['a' => true, 'b' => 3],
            null,
            ['default_context' => ['c' => 3, 'a' => true], 'name_converter' => 'some.converter'],
            $nameConverterId = 'serializer.name_converter.metadata_aware.'.ContainerBuilder::hash('some.converter'),
            ContainerBuilder::hash([['a' => true, 'c' => 3], $nameConverterId]),
        ];
    }

    /**
     * @testWith [{"a": true, "b": 3}, {"b": 3, "a": true}, null]
     *           [{}, {}, "some.converter"]
     *           [{"a": true, "b": 3}, {"b": 3, "a": true}, "some.converter"]
     */
    public function testMultipleNamedSerializersUseSameNewServicesForSameConfigs(
        array $defaultContext1,
        array $defaultContext2,
        ?string $nameConverter,
    ) {
        $container = new ContainerBuilder();
        $container->setParameter('kernel.debug', false);
        $container->setParameter('serializer.default_context', []);
        $container->setParameter('.serializer.name_converter', null);
        $container->setParameter('.serializer.named_serializers', [
            'api' => ['default_context' => $defaultContext1, 'name_converter' => $nameConverter],
            'api2' => ['default_context' => $defaultContext2, 'name_converter' => $nameConverter],
        ]);
        ...
        $this->assertNotEquals($container->getDefinition('serializer')->getArgument(0), $container->getDefinition('serializer.api')->getArgument(0));
        $this->assertNotEquals($container->getDefinition('serializer')->getArgument(1), $container->getDefinition('serializer.api')->getArgument(1));

        $this->assertEquals($container->getDefinition('serializer.api')->getArgument(0), $container->getDefinition('serializer.api2')->getArgument(0));
        $this->assertEquals($container->getDefinition('serializer.api')->getArgument(1), $container->getDefinition('serializer.api2')->getArgument(1));
    }

    public function testNamedSerializersDoNotCreateNewServicesWhenServiceDoesNotUseANameConverter()
    {
        $container = new ContainerBuilder();
        $container->setParameter('kernel.debug', false);
        $container->setParameter('.serializer.name_converter', null);
        ...
        $this->assertEquals($container->getDefinition('serializer')->getArgument(0), $container->getDefinition('serializer.api')->getArgument(0));
        $this->assertEquals($container->getDefinition('serializer')->getArgument(1), $container->getDefinition('serializer.api')->getArgument(1));
    }
    ...
    public function testBindSerializerDefaultContextToNamedSerializers()
    {
        ...
        $bindings = $container->getDefinition('n1.'.ContainerBuilder::hash([$defaultContext]))->getBindings();
    ...
    public function testNormalizersAndEncodersAreDecoratedAndOrderedWhenCollectingDataForNamedSerializers()
    {
        ...
        $hash = ContainerBuilder::hash([$defaultContext]);
        $traceableNormalizerDefinition = $container->getDefinition('.debug.serializer.normalizer.n.'.$hash);
        $traceableEncoderDefinition = $container->getDefinition('.debug.serializer.encoder.e.'.$hash);
        ...
        $this->assertEquals(TraceableNormalizer::class, $traceableNormalizerDefinition->getClass());
        $this->assertEquals(new Reference('n.'.$hash), $traceableNormalizerDefinition->getArgument(0));
        $this->assertEquals(new Reference('serializer.data_collector'), $traceableNormalizerDefinition->getArgument(1));
        $this->assertSame('api', $traceableNormalizerDefinition->getArgument(2));

        $this->assertEquals(TraceableEncoder::class, $traceableEncoderDefinition->getClass());
        $this->assertEquals(new Reference('e.'.$hash), $traceableEncoderDefinition->getArgument(0));
        $this->assertEquals(new Reference('serializer.data_collector'), $traceableEncoderDefinition->getArgument(1));
